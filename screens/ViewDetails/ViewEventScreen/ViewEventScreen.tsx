...
const ViewEventScreen = () => {
  const { onHostPress, eventUid } = useRoute<ViewEventScreenRouteProps>().params
  ...
  useEffect(() => {
    const fetchEventData = async () => {
      try {
        ...
        showErrorToast(
          "Error fetching event data. Please check your connection and try again."
        )
      }
    }
    fetchEventData()
      ...
      <View style={viewDetailsStyles.detailsContainer}>
        ...
        {event.host !== user?.uid && (
          ...
          >
            ...
            <ProfilePicture size={25} pictureUrl="" />
        ...
        {user?.uid === event.host && (
          <View style={styles.participateButton}>
            <Text style={globalStyles.boldText}>Hosted by you</Text>
          </View>
        )}
      </View>
    </View>
